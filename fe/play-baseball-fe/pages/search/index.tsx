...
const SearchPage: React.FC = () => {
  ...
  const [keyword, setKeyword] = useState<string | null>(null); // 초기에는 null로 설정
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const keywordFromQuery = query.get("keyword") || ""; // 기본값 설정
  ...
  const fetchItems = async (pageNumber: number) => {
    ...
    try {
      const response = await axios.get(
        `${EXCHANGE_SEARCH}?page=${pageNumber}&status=${status}&keyword=${keyword}`
      );
