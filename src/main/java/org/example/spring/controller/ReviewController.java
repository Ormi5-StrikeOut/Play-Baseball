...
public class ReviewController {
    ...
    public ResponseEntity<CreateReviewRequest> addReview(Member member, @RequestPart("review") CreateReviewRequest createReviewRequest, @RequestPart(value = "images", required = false) List<MultipartFile> images) throws IOException {
        reviewService.addReview(member, createReviewRequest, images);
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<Void> modifyReview(@PathVariable Long id, Member member, @Valid @RequestPart(value = "review", required = false) ModifyReviewRequest modifyReviewRequest, @RequestPart(value = "images", required = false) List<MultipartFile> images) throws IOException {
        reviewService.modifyReview(id, member, modifyReviewRequest, images);
    ...
    public ResponseEntity<Void> deleteReviewImage(@PathVariable Long reviewId, @PathVariable Long imageId) throws IOException {
        reviewService.deleteReviewImage(reviewId, imageId);
        return ResponseEntity.ok().build();
    }
}
